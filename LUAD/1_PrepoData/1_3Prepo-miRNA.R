...
explo.plot(mycountsbio, plottype = "barplot", samples = 1:1)
dev.off()
...
#241 features are to be kept for differential expression analysis with filtering method 1
...
designExp2 <- designExp[unique(designExp$samples),]
rownames(designExp) <- colnames(FilteredMatrix)
deseqFactors=DESeqDataSetFromMatrix(countData = FilteredMatrix, colData = designExp,
                                    design = ~1)
#deseqFactors=estimateSizeFactors(DESeqDataSetFromMatrix(newFilteredMatrix, colData = designExp))
deseqFactors=estimateSizeFactors(deseqFactors)
